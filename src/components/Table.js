...
const Table = ({fetcheddata,total,perpage,page,setPage,loading}) => {
  ...
  return (
    loading ? (
      <SkeletonTheme color="#e84118" highlightColor="#f3f3f3">
  ...
  <tbody>
    ...
    <tr key={e.id}>
      ...
      <td className='table-light'>{e.description}</td>
      <td className='table-info'>{e.price}</td>
      <td className='table-success'>{e.category}</td>
      <td className='table-danger'>{e.sold.toString()}</td>
      <td> <div>
